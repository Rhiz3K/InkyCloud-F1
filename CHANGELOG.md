...
- Preserve the country flag and qualifying/race columns for new circuits. Show podium
  dashes, blank year/times and a localized bordered badge with a hard shadow. Keep footer rows
  inside the display even without a flag and invalidate calendars cached with the previous layout.
- Show Malaysia as `MY` in the mobile race selector instead of the fallback flag symbol.
