...
- **Static-analysis hygiene cleanup** - Reworked the config test-reset helper, analytics/standings/version logging calls, database cursor context-manager nesting, stats cleanup query whitelisting, and ASGI middleware constructor names so the branch stays clean under DeepSource Python review without changing runtime behavior
...
- **Hourly stats history source-of-truth** - Switched `/api/stats/history` to derive hourly and rolling-24h counts directly from persisted `api_calls` so historical stats work against existing production data even when the legacy `request_stats` snapshot table is empty
...
- **Zero-valued perf aggregates** - Preserved `0.0` average vitals in page and trend stats payloads instead of collapsing them to `None`, so perfect CLS/LCP/FCP/TTFB samples remain visible in operational dashboards
