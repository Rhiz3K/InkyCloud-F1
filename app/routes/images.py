...
def _get_pregenerated_calendar_path(
    *,
    ...
    if target_tz == config.DEFAULT_TIMEZONE:
        # Default-tz variants come from the precomputed whitelist (no user input in the path).
        filename = _VALID_CALENDAR_FILENAMES.get((safe_lang, safe_display, safe_weather))
        if not filename:
            return None
    else:
        # Popular non-default timezones are pregenerated by the scheduler. tz reaches here only
        # after _validate_timezone_param/normalize_timezone, so it is a known IANA name and a safe
        # filename component; re-check defensively and reuse the scheduler's key builder so the
        # lookup matches the generated filename exactly.
        if not is_valid_timezone(target_tz):
            return None
        key_weather = "race" if safe_weather in ("race", "race_day") else safe_weather
...
async def _render_calendar(
    *,
    ...
    if not race_data:
        ...
        error_bmp = await asyncio.to_thread(renderer.render_error, "Failed to fetch race data")
    ...
    renderer = _get_renderer(display, translator, lang)
    # render_calendar is CPU-bound (Pillow + pure-Python palette/packing). Offload it so a cache
    # miss doesn't block the event loop and stall every other in-flight request.
...
async def get_calendar_bmp(
    ...
    try:
        ...
        translator = get_translator(lang)
        renderer = _get_renderer(display, translator, lang)
        bmp_data = await asyncio.to_thread(renderer.render_error, str(exc))
